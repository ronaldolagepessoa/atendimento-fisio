...
**Goal:** Substituir a grade semanal atual por um calendário estilo Google Calendar com eixo Y de horários (07h–18h, slots de 30 min) e eixo X de Seg–Sáb.

**Architecture:** Três mudanças cirúrgicas: (1) novos helpers de slot em `lib/agenda.ts`, (2) prop `defaultDataHora` no modal de agendamento, (3) reescrita do grid em `AgendaClient.tsx`. Nenhuma mudança em actions, page, schema ou outros componentes.
...
| Arquivo | Mudança |
|---|---|
| `src/lib/agenda.ts` | Adicionar `slotIndex()` e `slotToLocalISO()` |
| `src/app/(app)/agenda/AgendamentoModal.tsx` | Adicionar prop `defaultDataHora?: string` |
| `src/app/(app)/agenda/AgendaClient.tsx` | Reescrita completa — novo grid calendário |

---

## Task 1: Helpers de slot em `lib/agenda.ts`

**Files:**
- Modify: `src/lib/agenda.ts`

- [ ] **Step 1: Adicionar `slotIndex` e `slotToLocalISO` ao final de `src/lib/agenda.ts`**
...
Esperado: sem erros em `lib/agenda.ts`.

- [ ] **Step 3: Commit**

```bash
git add src/lib/agenda.ts
git commit -m "feat: slotIndex e slotToLocalISO helpers em lib/agenda"
```

---

## Task 2: Prop `defaultDataHora` no `AgendamentoModal`

**Files:**
- Modify: `src/app/(app)/agenda/AgendamentoModal.tsx`

- [ ] **Step 1: Adicionar `defaultDataHora` ao tipo `Props`**

Localizar o tipo `Props` (linha 9–16) e adicionar a prop:
...
- [ ] **Step 2: Receber e usar `defaultDataHora` na desestruturação e no cálculo de `brtInputs`**

Localizar a função `AgendamentoModal` (linha 20) e adicionar `defaultDataHora` na desestruturação:
...
Localizar a linha que define `brtInputs` (linha 55) e substituir por:
...
```bash
git add src/app/\(app\)/agenda/AgendamentoModal.tsx
git commit -m "feat: prop defaultDataHora no AgendamentoModal para pré-preencher data/hora"
```

---

## Task 3: Reescrita do `AgendaClient.tsx`

**Files:**
- Modify: `src/app/(app)/agenda/AgendaClient.tsx`

- [ ] **Step 1: Substituir o conteúdo completo de `AgendaClient.tsx`**
...
Checar em `http://localhost:3000/agenda`:

1. Grid exibe Seg–Sáb com horários 07h–17:30 no eixo Y
2. Agendamentos aparecem no slot correto (linha corresponde ao horário)
3. Hoje destaca-se com círculo indigo no cabeçalho e fundo levemente colorido
4. Linha vermelha aparece na hora atual (se semana atual)
5. Hover em slot vazio mostra `+`
6. Click em slot vazio (ADMIN) abre modal com data e hora pré-preenchidos
7. Click em card existente (ADMIN) abre modal de edição
8. Navegação anterior/próxima/hoje funciona
9. Dois agendamentos no mesmo slot e horário ficam lado a lado
10. Legenda de fisios e status aparece no rodapé

- [ ] **Step 5: Commit**

```bash
git add src/app/\(app\)/agenda/AgendaClient.tsx
git commit -m "feat: agenda redesign — grid calendário com eixo de horários"
```
